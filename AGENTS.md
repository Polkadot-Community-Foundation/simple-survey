...
Survey DApp is a decentralized survey app on Polkadot — create surveys, collect responses, and view aggregated results, all on-chain. (React + Vite + TypeScript, embedded in a Polkadot host — Mobile, Desktop, or Web; signing is approved on Polkadot Mobile). Canonical agent guidance is in [CLAUDE.md](CLAUDE.md) — read that first.
