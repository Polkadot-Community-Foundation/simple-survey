...
> Summit network** (`@polkadot/surveys`, `survey.dot`, Summit RPC/gateway in
> `src/utils.ts` and `cdm.json`). The production deploy is an **operator-only**
> path (gated, Bulletin-authorized account; the `survey.dot` short name needs
...
account, a terminal, and a phone. It uses Simple Survey as the example, but
...
```sh
git clone https://github.com/<your-github-username>/simple-survey.git
cd simple-survey
```
...
The name this repo ships with (`@example/surveys`) belongs to the original
deployment, so publishing under it from your account will fail. Claim your
own:

Edit `contracts/surveys/Cargo.toml`:

```toml
[package.metadata.cdm]
package = "@<your-handle>/surveys"   # was: @example/surveys
```
...
- `src/utils.ts`: one occurrence (the `CONTRACT_KEY` constant)
- `README.md`: one occurrence (the `cdm install` example)

Find them all with:

```sh
grep -rn "@example/surveys" --include="*.ts" --include="*.toml" --include="*.json" --include="*.md" . | grep -v node_modules
```

> Note for Simple Survey specifically: all instances deployed under the
> shipped name resolve the same contract, so they share one survey index.
> By deploying your own contract you're opting out of that; your instance
> starts with no surveys, and only your instance (and mods of it) write to
> it. That's the right trade for learning the full deploy. Keep the
> original name in `cdm.json` instead if you'd rather read the shared
> upstream surveys and skip steps 3-5.
...
the network endpoints pinned explicitly and then auto-syncs `cdm.json` via
`npm run install:contracts`. Either works.)
...
metadata to Bulletin, (4) registers `@<your-handle>/surveys → (address,
...
```sh
cdm i -n paseo @<your-handle>/surveys
```

(`cdm i` is short for `cdm install`; `npm run install:contracts` does the
same with the endpoints pinned, and `npm run deploy` already ran it for
you.) This writes the registered address and ABI into `cdm.json`; your
frontend now points at your contract.
...
```sh
playground deploy --playground --moddable --domain simple-survey --signer phone
```
...
- `moddable: yes ... <repo url>` must point at **your fork**. It's
  ...
  `git remote set-url origin https://github.com/<you>/simple-survey.git`.
...
```sh
playground deploy --playground --moddable --domain simple-survey --signer dev --suri "<your secret URI>"
```
...
- Check the playground's **Leaderboard**: your account earned its deploy
  XP.
- Create a survey in your instance and submit a response. Both land on
  Bulletin as real uploads, with their CIDs recorded in *your* contract.
